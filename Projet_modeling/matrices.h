...
inline Mat4 scale(float s) { return glm::scale(Vec3(s,s,s));}
...
inline Vec3 vec_sum( const Vec3& v1, const Vec3& v2 ){
    return Vec3( v1.x + v2.x, v1.y + v2.y , v1.z + v2.z );
}
// substruct: v2 - v1
inline Vec3 vec_sub( const Vec3& v1, const Vec3& v2 ){
    return Vec3( v2.x - v1.x, v2.y - v1.y , v2.z - v1.z );
}
// divide: v = v / d
inline Vec3 vec_div( const Vec3& v, float d){
    return Vec3( v.x / d, v.y / d , v.z / d );
}

// divide: v = v * d
inline Vec3 vec_mul( const Vec3& v, float d){
    return Vec3( v.x * d, v.y * d , v.z * d );
}
